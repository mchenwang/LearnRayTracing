...
class BVH_Node : public Hittable {
    ...
    static bool bbcmp(const std::shared_ptr<Hittable>& a, const std::shared_ptr<Hittable>& b, size_t axis) {
        AABB b1, b2;
...
public:
    BVH_Node() = default;
    BVH_Node(const std::vector<std::shared_ptr<Hittable>>& objects, size_t start, size_t end, double time0, double time1) {
    ...
    bool scatter(Ray& ray_out, const hit_info& hit) const { return false; }
    bool bounding_box(const double, const double, AABB& output_box) const { output_box = box; return true; }
    ...
    bool hit(const Ray& ray, double t_min, double t_max, hit_info& ret) const {
        ...
        if (hit_left || hit_right) {
            if (std::dynamic_pointer_cast<Sphere>(left)) ret.obj = left;
            if (std::dynamic_pointer_cast<Sphere>(right)) ret.obj = right;
        }
